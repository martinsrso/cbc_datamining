...
for dire in os.listdir():
    if dire != "csv"and dire != "script.py":
        ...
        globs = [glob.glob(dire+"/mixed-integer*"),glob.glob(dire+"/baseCompleta*"),
            glob.glob(dire+"/matrixminaij>=0andmaxaij<=1binaryproblems*"),
            glob.glob(dire+"/matrixminaij>=0andmaxaij<=1problems*"),
            glob.glob(dire+"/matrixminaij>=-1000andmaxaij<=1000binaryproblems*"),
            glob.glob(dire+"/matrixminaij>=-1000andmaxaij<=1000problems*"),
        ...
        for x in range(len(globs)):
            with open("/home/rmartins/Downloads/cbc_datamining/models/bnb/csv/"+strs[x]+".csv", "a+") as f:
